...
def transform_box2token_large(box,img,v):
    ...
    return list_se
class Executor:
    def __init__(self, device: str = "cpu", box_representation_method: str = "crop", method_aggregator: str = "max", enlarge_boxes: int = 0, expand_position_embedding: bool = False, square_size: bool = False, blur_std_dev: int = 100, cache_path: str = None, mask_value = -100.0) -> None:
        IMPLEMENTED_METHODS = ["crop", "blur", "shade", "circle", "box"]
    ...
    def tensorize_inputs(self, caption: str, image: Image, boxes: List[Box], image_name: str = None, mask_dino = None) -> Tuple[List[torch.Tensor], torch.Tensor]:
        images = []
        ...
        if self.cache_path is None or any([not os.path.exists(os.path.join(self.cache_path, model_name, image_name, method_name+".pt")) for model_name in self.model_names for method_name in self.box_representation_method.split(',')]):
            if "box" in self.box_representation_method:
                mask_attn = True
                if mask_attn:
                    for i in range(len(boxes)):
                        ...
                        g, gauss = transform_box2token_base(box, image_i, 0.2)
                        mask = torch.zeros(197,197)
                        for index, j in enumerate(g):
                            mask[0, j[0]:j[1]] = gauss[index]
                        preprocessed_images = self.preprocess_image(image_i)
                        for j, img in enumerate(preprocessed_images):
                            images[j].append((img.unsqueeze(0).to(self.device), mask))

            if "circle" in self.box_representation_method:
                blur = True
                for i in range(len(boxes)):
                    image_i = image.copy()
                    box = (
                    ...
                    width = int(max(image_i.size) / 200)

                    if not blur:
                        draw = ImageDraw.Draw(image_i)
                        draw.ellipse([box[:2], box[2:]], outline="red",width=width)
                        preprocessed_images = self.preprocess_image(image_i)

                    if blur:
                        mask = Image.new('L', image_i.size, 0)
                        draw = ImageDraw.Draw(mask)
                        draw.ellipse([box[:2], box[2:]], outline="red", fill=255)
                        blurred = image_i.filter(ImageFilter.GaussianBlur(self.blur_std_dev))
                        blurred.paste(image_i, mask=mask)
                        draw1 = ImageDraw.Draw(blurred)
                        draw1.ellipse([box[:2], box[2:]], outline="red",width=width)
    ...
    @torch.no_grad()
    def __call__(self, caption: str, image: Image, boxes: List[Box], image_name: str = None, masks=None,caption_bank=[], mask_dino=None) -> torch.Tensor:
        #t = time.time()
        images = self.tensorize_inputs(caption, image, boxes, image_name, mask_dino)
        ...
        for model, images_t, model_name in zip(self.models, images, self.model_names):
            ...
            logits_per_image, logits_per_text, image_features, text_features, attn= self.call_model(model, images_t, text_tensor, image_features=image_features, text_features=text_features)
        ...
        score_subtracting = False
        if score_subtracting:
        ...
        return all_logits_per_text.view(-1), attn

class ClipExecutor(Executor):
    def __init__(self, clip_model: str = "ViT-B/32", device: str = "cpu", box_representation_method: str = "crop", method_aggregator: str = "max", enlarge_boxes: int = 0, expand_position_embedding: bool = False, square_size: bool = False, blur_std_dev: int = 100, cache_path: str = None,mask_value=None) -> None:
    ...
    def call_model(self, model: torch.nn.Module, images: torch.Tensor, text: torch.Tensor, image_features: torch.Tensor = None, text_features: torch.Tensor = None) -> torch.Tensor:
        if image_features is None:
            ...
            for index,i in enumerate(images):
                image_features, attn = model.encode_image(i[0], i[1])
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                image_features_list.append(image_features)
                attn_list.append(attn)
            image_features = torch.cat(image_features_list, dim=0)
            attn1 = torch.cat(attn_list, dim=0)

        if text_features is None:
            #print('computing text features')
            text_features = model.encode_text(text,mask=None)
            # normalized features
        ...
        return logits_per_image, logits_per_text, image_features, text_features, attn1
    ...
    def __call__(self, caption: str, image: Image, boxes: List[Box], image_name: str = None, masks=None,caption_bank=[], mask_dino=None) -> torch.Tensor:
        ...
        result = super().__call__(caption, image, boxes, image_name,masks,caption_bank, mask_dino)
        if self.expand_position_embedding:
