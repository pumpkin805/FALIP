...
class Environment:
    ...
    def filter(self,
        ...
        if self.masks is not None:
            masks = [self.masks[i] for i in candidate_indices]
            result_partial = self.executor(caption, self.image, boxes, image_name=self.image_name,masks=masks)
        else:
            result_partial, attn = self.executor(caption, self.image, boxes, image_name=self.image_name, caption_bank=caption_bank, mask_dino=mask_dino)
        ...
        return result.cpu().numpy(), attn.cpu().numpy()
