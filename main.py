...
if __name__ == "__main__":
    ...
    parser.add_argument("--box_representation_method", type=str, default="box", help="method of representing boxes as individual images (crop, blur, or both separated by a comma)")
    ...
    parser.add_argument("--mask_value", type=float, default=-100.0, help="value of mask")
    parser.add_argument("--text_prompt", type=str, default="a photo of <caption>", help=" ")
    ...
    #for datum in data:
        ...
        seg_mask = [ann["segmentation"] for ann in datum["anns"]]
        ...
        for sentence in datum["sentences"]:
            if args.detector_file:
                ...
                boxes = gold_boxes
            if "seg" in args.box_representation_method:
                env = Environment(img, boxes, executor, (args.mdetr is not None and not args.mdetr_given_bboxes), str(datum["image_id"]), mask=seg_mask)
            else:
            ...
            if args.shuffle_words:
                ...
                result = method.execute(sentence["raw"].lower(), env, mask_dino = None)
            boxes = env.boxes
            correct = False
