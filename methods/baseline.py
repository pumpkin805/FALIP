...
class Baseline(RefMethod):
    ...
    @overrides
    def execute(self, caption: str, env: "Environment",caption_bank: List[str]=[], mask_dino=None) -> Dict[str, Any]:
        chunk_texts = self.get_chunk_texts(caption)
        probs,attn = env.filter(caption, area_threshold = self.box_area_threshold, softmax=True, caption_bank=caption_bank, mask_dino=mask_dino)
